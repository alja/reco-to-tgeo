...
//   //--- define some media
   ...
   for (i=0; i<ncells; i++) {
      ...
      TGeoVolume* cass = new TGeoVolumeAssembly("HODLER");

      //row->AddNode(cell, ncells+i+1, new TGeoTranslation(0,ycell,0));
      //row->AddNode(cell,ncells-i,new TGeoTranslation(0,-ycell,0));
      cass->AddNode(cell, ncells+i+1, new TGeoTranslation(0,ycell,0));
      cass->AddNode(cell,ncells-i,new TGeoTranslation(0,-ycell,0));
